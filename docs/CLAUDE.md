# CLAUDE.md — Pravila razvoja za HomeOS plus
...
**Lokacija dokumentacije (Faza 9c):** sva dokumentacija živi u `docs/`, a u
korijenu projekta ostaje samo `README.md` i `CLAUDE.md` od nekoliko linija koji
uvozi ovaj fajl (`@docs/CLAUDE.md`) — Claude Code automatski čita `CLAUDE.md`
iz korijena, pa bi bez tog uvoza pravila prestala da se učitavaju sama.
Reference između dokumenata u `docs/` su na susjeda (`ROADMAP.md`), bez
`docs/` prefiksa.
...
javne portove 80/443 za sve njih). HomeOS plus se **ne** takmiči za te
...
lang/
  ...
  en/, de/                  <- isti set ključeva (Faza 9b, test parnosti)
```
...
- Aplikacija je primarno na bosanskom (`APP_LOCALE=bs`), a od Faze 9b govori
  **tri jezika: `bs`, `en`, `de`**. Podržani jezici se dodaju na jednom mjestu:
  `App\Platform\Localization\Locales`.
- **Novi ključ ide u sva tri jezika istovremeno.** Test parnosti
  (`tests/Feature/Platform/LanguagesTest.php`) pada ako ključ postoji u jednom
  a fali u drugom — nedostajući prijevod se u UI-ju ne vidi kao greška nego kao
  sirovi ključ (`tasks.fields.title`).
- Prijevodi po modulu: `lang/bs/tasks.php`, `lang/en/tasks.php`,
  `lang/de/tasks.php`... — modul nosi svoje prijevode u istoj strukturi kao i
  ostatak koda, ne u jednom monolitnom fajlu.
- **Jezik zavisi od trenutka izvršavanja:** `__()` u kodu koji se izvršava pri
  registraciji service providera (npr. `->navigationGroups()` u
  `HomePanelProvider`) zamrzne prijevod, jer middleware koji postavlja jezik
  radi kasnije — takav tekst ide u closure.
- **Email ide na jeziku primaoca**, kroz `HasLocalePreference` na notifiable
  modelu (`User`, `HouseholdMember`) — ne na jeziku procesa koji ga šalje.
  Detalji i obrazloženja: `RULES.md` §1.
...
- **Terminologija i pravopis** (veliko/malo slovo, dosljedni termini dugmadi,
  formati datuma, prazna stanja) su fiksirani u `RULES.md` —
...
- [ ] Prati custom Filament temu i responzivnost/pristupačnost pravila iz
      ...
      imaju smislen tekst
- [ ] Prijevodi u `lang/bs/<modul>.php`, `lang/en/...`, `lang/de/...` — isti
      set ključeva u sva tri (tačka 13)
- [ ] Edit stranice koriste `App\Platform\Filament\Concerns\CancelReturnsToList`
      — „Nazad“ vodi na listu, ne na prethodnu stranicu (`RULES.md` §9)
...
- Ne dirati postojeće produkcijske vhost-ove na serveru (server već
  ...
  interna, ne navodi se u ovom javnom repou) — HomeOS plus ima svoje izolovane
...
- Ne pisati hardkodovan tekst u Filament UI — sve ide kroz `lang/<jezik>/*`.
- Ne dodavati prijevodni ključ u samo jedan jezik — test parnosti pada, i to
  je namjerno (tačka 13).
...
`ORIGINAL_SPEC.md` — izvorni brief nosi namjeru koju su ROADMAP/CLAUDE/
